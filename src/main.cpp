...
// --- LwIP & NAPT Headers ---
// These headers connect us to the ESP-IDF network stack
#include "lwip/napt.h"
...
void ppp_status_cb(ppp_pcb *pcb, int err_code, void *ctx) {
    ...
    switch (err_code) {
        case PPPERR_NONE: {
            ...
            Serial.print("   IP: "); Serial.println(ip4addr_ntoa(netif_ip4_addr(pppif)));
            Serial.print("   GW: "); Serial.println(ip4addr_ntoa(netif_ip4_gw(pppif)));
            Serial.print("   MS: "); Serial.println(ip4addr_ntoa(netif_ip4_netmask(pppif)));
...
void modemInit() {
    ...
    delay(100);
    digitalWrite(MODEM_PWRKEY, HIGH); // Power Key is usually Active Low or High pulse, T-SIM7600 is Active HIGH pulse for ON
    delay(1000); // 1s Pulse
    digitalWrite(MODEM_PWRKEY, LOW);
